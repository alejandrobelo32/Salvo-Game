...
$("#btnAddShips").click(function(){
  ...
  var ships=types.map(function(type){
...
function getLocations(type){
  ...
  let x = parseInt($(`#${type}`).attr("data-gs-x"))+1;
  let y = parseInt($(`#${type}`).attr("data-gs-y"))+1;

  let height = parseInt($(`#${type}`).attr("data-gs-height"));
  let width = parseInt($(`#${type}`).attr("data-gs-width"));
